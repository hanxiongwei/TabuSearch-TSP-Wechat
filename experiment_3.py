...
description：
  based on TSP model,
  compare the tabu search with couple tabu stratery and tabu search with single tabu stratery
  output the result
"""
...
def solve(inst):
    ...
    def tabu_single_solve(ini_route, iteration):
        tabu_list = [0 for i in range(n)]
        tabu_length = int(len(ini_route) * 0.2)
        ...
        for k in range(iteration):
            ...
            for neighbor in sorted_neighbors:
                ...
                if(temp_local < best):
                    ...
                    if(tabu_list[nid_i] != 0 or tabu_list[nid_j] != 0):
                        continue
                    else:
                        local = temp_local
                break
            ...
            for i in range(n):
                    if(tabu_list[i] != 0):
                        tabu_list[i] -= 1
            tabu_list[nid_i] = tabu_length
            tabu_list[nid_j] = tabu_length
            #print(nid_i, nid_j)
            #print(tabu_list)
    ...
    #%%
    def tabu_couple_solve(ini_route, iteration):
    ...
    #%%
    result = dict()
    result['tabu_single_search'] = tabu_single_solve(ini_route, iteration)
    result['tabu_couple_search'] = tabu_couple_solve(ini_route, iteration)
    return result
...
def multy():
    ...
    #存储结果
    f = file('result/experiment_2.pkl', 'wb')
    pickle.dump(result, f, True)
    f.close()
    
    #%%
    f = file('result/experiment_2.pkl', 'rb')  
    result = pickle.load(f)
    f.close()
    #%%
    #处理数据，输出为csv文件
    epsilon = 0.000001
    table = [['node_count'], ['tabu_single_search'], ['tabu_couple_search'], ['avg_gap']]
    for node_count in node_count_list:
        ...
        for i in range(experiment_count):
            ...
            ls_obj = temp_result['tabu_single_search']['cost']
            ts_obj = temp_result['tabu_couple_search']['cost']
    ...
    print(table)
    #%%
    f = open('result/experiment_2.csv', 'wb')
    writer = csv.writer(f)
    for line in table:
...
def analysis():
    ...
    values = result[str(node_count) + ',1']['tabu_single_search']['iteration_values']
    plt.plot(x, values)
    values = result[str(node_count) + ',1']['tabu_couple_search']['iteration_values']
    plt.plot(x, values)
